...
void Aut::trkstr(const string& lvarIdxStr)
{
    // Add Integer Variable (represent a string)
    size_t lvarIdx = stoi(lvarIdxStr);
    addLVar(lvarIdx);
    string lv  = piList[LEN  ][lvarIdx]->_name;
    string lvn = piList[LEN_N][lvarIdx]->_name;
    ...
    #ifndef AUT_OP_NDEBUG
        cout << "[Aut::trkstr] " << _name << " lvar=" << lvarIdxStr << endl;
    #endif

    // Add String Constraint
    renameITO("tmpT", getT());
    defineFun("tmp1", "(and (not epsilon) (= " + lvn + " (+ (* " + lv + " 257) (+ (+ (ite x0 128 0) (+ (ite x1 64 0) (+ (ite x2 32 0) (+ (ite x3 16 0) (+ (ite x4 8 0) (+ (ite x5 4 0) (+ (ite x6 2 0) (ite x7 1 0)))))))) 1))))", _imdList);
    defineFun("tmp2", "(and epsilon (= " + lvn + " " + lv + "))", _imdList);
    defineFun("tmp3", "(and tmpT (or tmp1 tmp2))", _imdList, &Aut::setT);
    renameDef();
}
